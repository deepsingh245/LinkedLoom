...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    ...
    try {
      await api.firebaseService.registerWithEmailAndPassword(
        data.email,
        data.password,
        null
      );
  ...
  const loginWithLinkedin = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/getLinkedInAuthUrl`
      );
    ...
    <div className="relative flex items-center justify-center min-h-screen bg-[#080808] overflow-hidden p-4">
      ...
      <Card className="w-full max-w-md shadow-2xl border-[#1e1e2a] bg-[#13131a]/80 backdrop-blur-xl fade-up relative z-10">
        <CardHeader className="space-y-1.5 pb-6">
          ...
          <CardTitle className="text-[28px] font-display font-semibold tracking-tight text-center text-[#f0f0f8]">
            Create Account
          </CardTitle>
        ...
        <CardFooter className="text-sm text-center block text-[#8888a0]">
          ...
          <Link href={Routes.LOGIN} className="font-semibold text-[#63d496] hover:underline">
            Sign in
          </Link>
        </CardFooter>
      </Card>
